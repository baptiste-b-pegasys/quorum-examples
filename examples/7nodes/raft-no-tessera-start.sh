...
do
    ...
    PRIVATE_CONFIG=ignore nohup geth --datadir qdata/dd${i} ${ARGS} ${permissioned} --raftport ${raftPort} --http.port ${rpcPort} --port ${port} --plugins file://./plugins/geth-plugin-settings.json --plugins.skipverify 2>>qdata/logs/${i}.log &
done
