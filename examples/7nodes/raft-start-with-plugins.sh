...
ARGS="--nodiscover --nousb ${allowSecureUnlock} --verbosity ${verbosity} --networkid $NETWORK_ID --raft --raftblocktime ${blockTime} --http --http.corsdomain=* --http.vhosts=* --http.addr 0.0.0.0 --http.api admin,eth,debug,miner,net,shh,txpool,personal,web3,quorum,raft,quorumPermission,quorumExtension,plugin@helloworld --emitcheckpoints --unlock 0 --password passwords.txt $QUORUM_GETH_ARGS"
...
do
    ...
    PRIVATE_CONFIG=qdata/c${i}/tm.ipc nohup geth --datadir qdata/dd${i} ${ARGS} ${permissioned} --raftport ${raftPort} --http.port ${rpcPort} --port ${port} --plugins file://./plugins/geth-plugin-settings.json --plugins.skipverify 2>>qdata/logs/${i}.log &
done
